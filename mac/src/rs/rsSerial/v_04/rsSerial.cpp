...
bool    rsSerial::g_tx_enabled = true;
...
void rsSerial::Tx       (uint8_t* p_CharBufferTx)  {
    if(g_tx_enabled) uyProtocol::send_blocking(p_CharBufferTx);
}
...
void rsSerial::Tx       (char* p_CharBufferTx, uint8_t p_SizeBuf)  {
    if(g_tx_enabled) uyProtocol::send_blocking((uint8_t*) p_CharBufferTx, p_SizeBuf);
}

// ******************************************************************************************
// ******************************************************************************************
void rsSerial::Tx       (uint8_t* p_CharBufferTx, uint8_t p_SizeBuf, uint8_t p_length)  {
    if(g_tx_enabled) uyProtocol::send_blocking(p_CharBufferTx, p_SizeBuf, p_length);
}
...
void rsSerial::RxStop      (void)   {
    sdk_usart::rx_stop();
    while(!uyProtocol::status_get_rx__free()){}
}
