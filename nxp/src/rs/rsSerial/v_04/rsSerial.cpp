...
void rsSerial::RxStart       (uint8_t* p_CharBufferTx, size_t p_SizeOf)  {
	...
	if(SDK_USART_RX_STATUS__FREE == t_handle_demo_marco.Status) {
...
bool rsSerial::RxCheck     (void)   {
    if(SDK_USART_RX_STATUS__READY_TO_READ == t_handle_demo_marco.Status) {
        t_handle_demo_marco.Status = SDK_USART_RX_STATUS__FREE;
        return true;
    }
