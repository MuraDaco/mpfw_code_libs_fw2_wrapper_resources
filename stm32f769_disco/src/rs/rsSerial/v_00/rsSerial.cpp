...
// sdk
#include "sdk_config/stm_main.h"
#include "sdk_config/stm_main_private.h"
...
void rsSerial::Tx       (char const *p_CharBufferTx,  uint8_t p_SizeBuf)  {
    ...
    int l_status = HAL_UART_Transmit_DMA(&UartHandle, (uint8_t* const) p_CharBufferTx, p_SizeBuf);
    if(HAL_OK != l_status)
    {
        Error_Handler();
    }
...
int rsSerial::TxFreeRTOS             (char* p_Buffer, uint8_t p_SizeBuf)      {
    // ******************************************************************************************
    TxEnable();

    int l_status = HAL_UART_Transmit_DMA(&UartHandle, (uint8_t*) p_Buffer, p_SizeBuf);
    if(HAL_OK != l_status)
    {
        Error_Handler();
    }
...
int rsSerial::TxFreeRTOS             (const char* p_Buffer, uint8_t p_SizeBuf)      {
    // ******************************************************************************************
    TxEnable();
...
int rsSerial::RxFreeRTOS             (char* p_Buffer, uint8_t p_SizeBuffer, uint8_t* p_ptrByteRxCounter)      {
	...
	int l_status = HAL_UART_Receive_DMA(&UartHandle, (uint8_t *)p_Buffer, p_SizeBuffer);
    if(HAL_OK != l_status)
    {
        Error_Handler();
    }
