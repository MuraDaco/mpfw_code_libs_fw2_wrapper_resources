...
#define DEMO_MARCO_USART_REG_PTR                UartHandle.Instance
...
void rsSerial::RxStart       (uint8_t* p_CharBufferTx, size_t p_SizeOf, uint8_t p_packet_length)  {
    ...
    if(uyProtocol::status_get_rx__free())   {
        // ------------------------------
        RxEnable();
        // USART_ReadBlocking(DEMO_MARCO_USART_REG_PTR, p_CharBufferTx, p_packet_length);
        HAL_UART_Receive(&UartHandle, p_CharBufferTx, p_packet_length,HAL_MAX_DELAY);
