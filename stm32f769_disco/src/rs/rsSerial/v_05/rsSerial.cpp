...
void rsSerial::Tx       (uint8_t* p_CharBufferTx)  {
    ...
    int l_status = HAL_UART_Transmit_IT(&UartHandle, p_CharBufferTx, l_SizeBuf);
    if(HAL_OK != l_status)
    {
        Error_Handler();
    }
...
void rsSerial::Tx       (char* p_CharBufferTx)  {
    ...
    int l_status = HAL_UART_Transmit_IT(&UartHandle, (uint8_t* ) p_CharBufferTx, l_SizeBuf);
    if(HAL_OK != l_status)
    {
        Error_Handler();
    }
...
void rsSerial::Tx       (char const *p_CharBufferTx)  {
    ...
    int l_status = HAL_UART_Transmit_IT(&UartHandle, (uint8_t* const) p_CharBufferTx, l_SizeBuf);
    if(HAL_OK != l_status)
    {
        Error_Handler();
    }
...
void rsSerial::Tx       (char* p_CharBufferTx, uint8_t p_SizeBuf)  {
    ...
    int l_status = HAL_UART_Transmit_IT(&UartHandle, (uint8_t *) p_CharBufferTx, p_SizeBuf);
    if(HAL_OK != l_status)
    {
        Error_Handler();
    }
...
void rsSerial::Tx       (uint8_t* p_CharBufferTx, uint8_t p_SizeBuf, uint8_t p_length)  {
    ...
    int l_status = HAL_UART_Transmit_IT(&UartHandle, p_CharBufferTx, p_length);
    if(HAL_OK != l_status)
    {
        Error_Handler();
    }
...
void rsSerial::RxStart       (uint8_t* p_CharBufferTx, size_t p_SizeOf, uint8_t p_packet_length)  {
	...
	int l_status = HAL_UART_Receive_IT(&UartHandle, p_CharBufferTx, p_packet_length);
    if(HAL_OK != l_status)
    {
        Error_Handler();
    }
...
void rsSerial::RxStart       (uint8_t* p_CharBufferTx, size_t p_SizeOf)  {
	...
	int l_status = HAL_UART_Receive_IT(&UartHandle, p_CharBufferTx, p_SizeOf);
    if(HAL_OK != l_status)
    {
        Error_Handler();
    }
...
void rsSerial::RxStart       (char* p_CharBufferTx, size_t p_SizeOf)  {
	...
	int l_status = HAL_UART_Receive_IT(&UartHandle, (uint8_t *)p_CharBufferTx, p_SizeOf);
    if(HAL_OK != l_status)
    {
        Error_Handler();
    }
...
char rsSerial::RxStart    (void)  {
	...
	int l_status = HAL_UART_Receive_IT(&UartHandle, &ch, 1);
    if(HAL_OK != l_status)
    {
        Error_Handler();
    }
...
bool rsSerial::RxCheck     (void)   {
    if(SDK_USART_RX_STATUS__READY_TO_READ == status)   {
